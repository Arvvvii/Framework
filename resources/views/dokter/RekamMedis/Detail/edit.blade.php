...
<div class="app-content-header">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6">
                <h3 class="mb-0">Edit Detail Tindakan/Terapi</h3>
            </div>
            <div class="col-sm-6">
...
<div class="app-content">
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('dokter.rekammedis.detail.update', [$rekamMedis->idrekam_medis, $detail->iddetail_rekam_medis, 'pet' => $petId]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        ...
                        <select name="idkode_tindakan_terapi" id="idkode_tindakan_terapi" class="form-control" required>
                            ...
                            @foreach($kodeTindakan as $kode)
                                <option value="{{ $kode->idkode_tindakan_terapi }}" {{ (old('idkode_tindakan_terapi', $detail->idkode_tindakan_terapi) == $kode->idkode_tindakan_terapi) ? 'selected' : '' }}>{{ $kode->kode }} - {{ $kode->deskripsi_tindakan_terapi }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="detail" class="form-label">Detail</label>
                        <textarea name="detail" id="detail" class="form-control" rows="3" required>{{ old('detail', $detail->detail) }}</textarea>
                    </div>
                    <div class="text-end">
